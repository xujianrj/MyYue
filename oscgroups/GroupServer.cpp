...
int GroupServer::GetAllGroupNames(std::vector<std::string>& vGroupNames)
{
    vGroupNames.clear();
    group_iterator group_iter = groups_.begin();
    for(; group_iter != groups_.end(); ++group_iter)
    {
        std::string groupName = group_iter->first;
        vGroupNames.push_back(groupName);
    }

    return vGroupNames.size();
}
