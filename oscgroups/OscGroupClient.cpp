...
bool IsGroupInInterestingList(const char* groupname);

GroupChangeHandler g_GroupChangeHandler = NULL;
UserChangeHandler  g_UserChangeHandler = NULL;
GroupClient* g_InterestringGroups = NULL;
int g_InterestringGroupsCount = 0;

void SetGroupChangeHandler(GroupChangeHandler callback)
{
    g_GroupChangeHandler = callback;
}

void SetUserChangeHanlder(UserChangeHandler callback)
{
    g_UserChangeHandler = callback;
}

void SetInterestingGroups(GroupClient* interestingGroups, int count)
{
    if(g_InterestringGroups)
    {
        delete [] g_InterestringGroups;
        g_InterestringGroupsCount = 0;
    }

    //std::cout << "intereting group set: " << std::endl;

    g_InterestringGroups = new GroupClient[count];
    memset(g_InterestringGroups, 0, count * sizeof(GroupClient));

    for(int i = 0; i < count; i++)
    {
        if(IsGroupInInterestingList(interestingGroups[i].group_name_))
        {
            continue;
        }

        memcpy(&g_InterestringGroups[g_InterestringGroupsCount],
                &interestingGroups[g_InterestringGroupsCount],
                sizeof(GroupClient));
        g_InterestringGroups[g_InterestringGroupsCount].group_name_[MAX_GROUPNAME_LEN] = 0;
        g_InterestringGroups[g_InterestringGroupsCount].group_password_[MAX_GROUPPWD_LEN] = 0;

       // std::cout << g_InterestringGroups[i].group_name_ << std::endl;
       // std::cout << g_InterestringGroups[i].group_password_ << std::endl;

        g_InterestringGroupsCount++;
    }

    //std::cout << "total:" << count << std::endl;
    //std::cout << "intereting group end" << std::endl;
}


bool IsGroupInInterestingList(const char* groupname)
{
     for(int i = 0; i < g_InterestringGroupsCount; i++)
     {
         std::string str1(g_InterestringGroups[i].group_name_ );
         std::string str2(groupname);
...
class ExternalCommunicationsSender : public TimerListener {
    ...
    void PrepareAliveBuffer()
    {
        ...
        p << osc::BeginMessage( "/groupserver/get_group_info" )
                << osc::EndMessage;

         //At least add its own group to interesting list
        if(!IsGroupInInterestingList(groupName_.c_str()))
        {
            memcpy(g_InterestringGroups[g_InterestringGroupsCount].group_name_,
                    groupName_.c_str(),
                    sizeof(g_InterestringGroups[g_InterestringGroupsCount].group_name_));
            memcpy(g_InterestringGroups[g_InterestringGroupsCount].group_password_,
                    groupPassword_.c_str(),
                    sizeof(g_InterestringGroups[g_InterestringGroupsCount].group_password_));
        ...
        if(g_InterestringGroupsCount != 0)
        {
            p << osc::BeginMessage("/groupserver/get_user_list" );

            for(int i = 0; i < g_InterestringGroupsCount; i++)
            {
                char groupPasswordHash[33];
                MakeHashString( groupPasswordHash, g_InterestringGroups[i].group_password_ );

                p << g_InterestringGroups[i].group_name_;
                p << groupPasswordHash;
            }
            p << osc::EndMessage;
        }
...
class ExternalSocketListener : public osc::OscPacketListener
{
    ...
    void group_info( const osc::ReceivedMessage& m, const IpEndpointName& remoteEndpoint )
    {
        // only accept user_alive_status from the server
        if( remoteEndpoint != remoteServerEndpoint_ )
            return;
        ...
        memset(groupsOnServer_, 0, sizeof(GroupClient) * MAX_GROUP_ON_CLIENT);
        groupOnServerCount = 0;
        ...
        while( !args.Eos() )
        {
            ...
            int maxLen =  (strlen(groupName) > MAX_GROUPNAME_LEN) ? MAX_GROUPNAME_LEN : strlen(groupName);
            memcpy(groupsOnServer_[groupOnServerCount].group_name_, groupName, maxLen);
            groupsOnServer_[groupOnServerCount].group_name_[MAX_GROUPNAME_LEN] = 0;

            groupOnServerCount++;
        }

        if(g_GroupChangeHandler)
        {
            g_GroupChangeHandler(groupsOnServer_, groupOnServerCount);
        }
    }

    void user_list( const osc::ReceivedMessage& m, const IpEndpointName& remoteEndpoint )
    {
        if( remoteEndpoint != remoteServerEndpoint_ )
            return;
       ...
       // std::cout << "user begin" << std::endl;

        memset(userList_, 0, MAX_USER_ON_CLIENT * sizeof(UserClient));
        int userListCount_ = 0;
        while( !args.Eos() )
        {
            const char* userName;
            const char* groupName;
            ...
            char privateAddressString[ IpEndpointName::ADDRESS_STRING_LENGTH ];
            privateEndpoint.AddressAsString( privateAddressString );
            char publicAddressString[ IpEndpointName::ADDRESS_STRING_LENGTH ];
            publicEndpoint.AddressAsString( publicAddressString );

            int maxLen = (strlen(userName) > MAX_USERNAME_LEN ) ?  MAX_USERNAME_LEN :strlen(userName);
            memcpy(userList_[userListCount_].user_name_, userName, maxLen);
            userList_[userListCount_].user_name_[MAX_USERNAME_LEN] = 0;

            maxLen = (strlen(groupName) > MAX_GROUPNAME_LEN) ? MAX_GROUPNAME_LEN : strlen(groupName);
            memcpy(userList_[userListCount_].group_name_, groupName, maxLen);
            userList_[userListCount_].group_name_[MAX_GROUPNAME_LEN] = 0;

            maxLen = (strlen(privateAddressString) > MAX_IP_LEN) ? MAX_IP_LEN : strlen(privateAddressString);
            memcpy(userList_[userListCount_].private_ip, privateAddressString, maxLen);
            userList_[userListCount_].private_ip[MAX_IP_LEN] = 0;

            maxLen = (strlen(publicAddressString) > MAX_IP_LEN) ? MAX_IP_LEN : strlen(publicAddressString);
            memcpy(userList_[userListCount_].public_ip, publicAddressString, maxLen);
            userList_[userListCount_].public_ip[MAX_IP_LEN] = 0;

            char pri_port[MAX_PORT_LEN + 1];
            char pub_port[MAX_PORT_LEN + 1];
            memset(pri_port, 0, sizeof(pri_port));
            memset(pub_port, 0, sizeof(pub_port));

            std::sprintf( pri_port, "%d", privatePort );
            std::sprintf( pub_port, "%d", publicPort );

            maxLen = (strlen(pri_port) > MAX_PORT_LEN) ? MAX_PORT_LEN : strlen(pri_port);
            memcpy(userList_[userListCount_].private_port, pri_port, maxLen);
            userList_[userListCount_].private_port[MAX_PORT_LEN] = 0;

            maxLen = (strlen(pub_port) > MAX_PORT_LEN) ? MAX_PORT_LEN : strlen(pub_port);
            memcpy(userList_[userListCount_].public_port, pub_port, maxLen);
            userList_[userListCount_].public_port[MAX_PORT_LEN] = 0;

            userListCount_++;
        }

        if(g_UserChangeHandler)
        {
            g_UserChangeHandler(userList_, userListCount_);
        }

      //  std::cout << "user end" << std::endl;
    }
...
protected:
			...
			const IpEndpointName& remoteEndpoint )
    {
        try{
            ...
            {
                std::cout << "get response: /groupclient/user_info" << std::endl;
                user_info( m, remoteEndpoint );
            }
            else if( std::strcmp( m.AddressPattern(), "/groupclient/ping" ) == 0 )
            {
                std::cout << "get response: /groupclient/ping" << std::endl;
                ping( m, remoteEndpoint );
            }
            else if( std::strcmp( m.AddressPattern(), "/groupclient/user_alive_status" ) == 0 )
            {
                std::cout << "get response: /groupclient/user_alive_status" << std::endl;
                user_alive_status( m, remoteEndpoint );
            }
            else if( std::strcmp( m.AddressPattern(), "/groupclient/user_group_status" ) == 0 )
            {
                std::cout << "get response: /groupclient/user_group_status" << std::endl;
                user_group_status( m, remoteEndpoint );
            }
            else if( std::strcmp( m.AddressPattern(), "/groupclient/group_info" ) == 0 )
            {
                std::cout << "get response: /groupclient/group_info" << std::endl;
                group_info( m, remoteEndpoint );
            }
            else if( std::strcmp( m.AddressPattern(), "/groupclient/user_list" ) == 0 )
            {
                std::cout << "get response: /groupclient/user_list" << std::endl;
                user_list( m, remoteEndpoint );
            }
    ...
    //std::vector<GroupClient*> groupsOnServer_;
    GroupClient groupsOnServer_[MAX_GROUP_ON_CLIENT];
    int groupOnServerCount;
    UserClient userList_[MAX_USER_ON_CLIENT];
    int userListCount_;
...
void OnGroupChange(GroupClient* groups, int count)
{
    if(groups != NULL)
    {
        ...
        for(int i = 0; i < count; i++)
        {
            std::cout << groups[i].group_name_ << std::endl;
        }
...
void OnUserChange(UserClient* users, int count)
{
    if(users != NULL)
    {
        std::cout << "users begin: " << std::endl;
        for(int i = 0; i < count; i++)
        {
            std::cout << "UserName:" << users[i].user_name_ << ", "
                      << "GroupName:" << users[i].group_name_ << ", "
                      << "PrivateIp:" << users[i].private_ip << ", "
                      << "PrivatePort:" << users[i].private_port << ", "
                      << "PublichIp:" << users[i].public_ip << ", "
                      << "PublicPort:" << users[i].public_port << std::endl;
        }

        std::cout << "users end" << std::endl;
    }
}
...
//{
//    std::cout << "run from main" << std::endl;
//    SetGroupChangeHandler(OnGroupChange);
//    SetUserChangeHanlder(OnUserChange);
//
//    char szGroupName[] = "wwwg";
//    char szGroupPwd[] = "wwwgp";
//    char szGroupName2[] = "mmmg";
//    char szGroupPwd2[] = "mmmgp";
//    GroupClient* pInterestingGroups = new GroupClient[2];
//    memset(pInterestingGroups, 0, sizeof(GroupClient)*2);
//    memcpy(pInterestingGroups[0].group_name_, szGroupName, sizeof(char*) * strlen(szGroupName));
//    memcpy(pInterestingGroups[0].group_password_, szGroupPwd, sizeof(char*) * strlen(szGroupPwd));
//    memcpy(pInterestingGroups[1].group_name_, szGroupName2, sizeof(char*) * strlen(szGroupName2));
//    memcpy(pInterestingGroups[1].group_password_, szGroupPwd2, sizeof(char*) * strlen(szGroupPwd2));
//
//    SetInterestingGroups(pInterestingGroups, 2);
//
//    int ret = oscgroupclient_main( argc, argv );
//
//    delete [] pInterestingGroups;
//    return ret;
//}

//TODO:
// 1. figure out how to ping each other?
// 2. figure out how to send message to each other.
// 3. if no interesting groups, at least interest my own group   //finish
