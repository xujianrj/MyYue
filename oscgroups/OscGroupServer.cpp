    ...
    , public TimerListener
{
    ...
    void MakeGroupInfoMessage( osc::OutboundPacketStream& p )
    {
        p << osc::BeginMessage( "/groupclient/group_info" );

        std::vector<std::string> vGroupNames;
        int size = groupServer_.GetAllGroupNames(vGroupNames);
        for(int i = 0; i < size; i++)
        {
            p << vGroupNames[i].c_str();
        }
    ...
    void get_group_info(const osc::ReceivedMessage& m,
            const IpEndpointName& remoteEndpoint )
    {
        (void) m;
        (void) remoteEndpoint; // suppress unused parameter warning
        MakeGroupInfoMessage( resultStream_ );
    }
...
protected:
			...
			const IpEndpointName& remoteEndpoint )
    {
        try
        {
            ...
            {
                std::cout << "get requirement: /groupserver/user_alive" << std::endl;
                user_alive( m, remoteEndpoint );
            }
            else if(std::strcmp(m.AddressPattern(), "/groupserver/get_group_info") == 0)
            {
                std::cout << "get requirement: /groupserver/get_group_info" << std::endl;
                 get_group_info(m, remoteEndpoint);
            }
            else if(std::strcmp(m.AddressPattern(), "/groupserver/get_user_list" ) == 0 )
            {
                std::cout << "get requirement: /groupserver/get_user_list" << std::endl;
                get_users_list( m, remoteEndpoint );
            }
            else if(std::strcmp(m.AddressPattern(), "/groupserver/get_group_users_info" ) == 0 )
            {
                std::cout << "get requirement: /groupserver/get_group_users_info" << std::endl;
